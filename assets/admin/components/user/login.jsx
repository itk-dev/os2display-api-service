...
function Login() {
  ...
  useEffect(() => {
    ...
    AdminConfigLoader.loadConfig().then((loadedConfig) => {
      setLoginMethods(loadedConfig.loginMethods ?? loginMethodDefaults);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    if (search) {
      ...
      if (state && code) {
        ...
        fetch(`/v2/authentication/oidc/token?${searchParams}`, {
          mode: "cors",
          credentials: "include",
        })
          .then((resp) => resp.json())
        ...
        <div className="login-page">
          <div className="login-container container">
            <Row className="login-box-shadow">
              <Col
                md="4"
                className="bg-dark col justify-content-between d-flex flex-column info-box"
              >
                <LoginSidebar />
              </Col>
